...
## Appendix: Role of `--` in Git

In git command, `--` is a seperator between options and paths. 
It is useful when a name could be ambiguous—e.g., if a branch or tag has the same name as a file.

If the path (file or directory) does not conflict with a branch or tag name, Git automatically treats it as a path.

::: {#exm- .custom_problem }
<br>

```bash
git diff feature -- file.txt
```

In the above command, `feature` could be interpreted as either a branch or a file.
Using `--` explicitly tells Git that anything after it is a path, not a branch or option.
...
Git sees that `README.md` is not a branch or tag, so it is interpreted as a file path.
Therefore, the command works correctly even without `--`.
